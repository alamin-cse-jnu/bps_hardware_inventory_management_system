...
def alerts_list(request):
    status_filter = _alert_status_filter(request)
...
# ── Office change alerts ──────────────────────────────────────────────────────

def _alert_status_filter(request) -> str:
...
@viewer_required
def office_changes_list(request):
    status_filter = _alert_status_filter(request)

    qs = OfficeChangeAlert.objects.select_related(
    ...
    counts = {
        "open": OfficeChangeAlert.objects.filter(status=AlertStatus.OPEN).count(),
        "resolved": OfficeChangeAlert.objects.filter(status=AlertStatus.RESOLVED).count(),
        "dismissed": OfficeChangeAlert.objects.filter(status=AlertStatus.DISMISSED).count(),
    }

    return render(request, "assignments/office_changes_list.html", {
        "alerts": qs,
        "status_filter": status_filter,
        "counts": counts,
    })
...
@it_officer_required
@require_http_methods(["GET", "POST"])
def office_change_panel(request, pk):
    alert = get_object_or_404(OfficeChangeAlert, pk=pk)
    ...
    if request.method == "POST":
        ...
        if action == "resolve":
            ...
            return render(request, "assignments/office_change_panel.html", {
                "alert": alert,
                "active_assignments": active_assignments,
                "error": "Unknown action.",
            })

        return render(request, "assignments/office_change_done.html", {
            "alert": alert,
            "action": action,
        })
